...
class KeyboardView @JvmOverloads constructor(
    ...
    private fun onKeyDraw(key: Keyboard.Key) {
        ...
        // Draw the numeric label above the main label
        val numericLabel = key.popupKeyboardChars.firstOrNull { it.isDigit() }
        if (numericLabel != null) {
            paint.textSize = labelTextSize * 0.6f // Decrease the size for the numeric label
            paint.color = keyTextColorSecondary
            // Получаем текущую ориентацию экрана
            val orientation = resources.configuration.orientation
            // Определяем координаты для отрисовки цифры
            val numericX = if (orientation == Configuration.ORIENTATION_LANDSCAPE) {
                // Для альбомной ориентации
                (key.width - keyBgPadding.left - keyBgPadding.right) / 2F + keyBgPadding.left + key.width * 0.15f
            } else {
                // Для портретной ориентации
                (key.width - keyBgPadding.left - keyBgPadding.right) / 2F + keyBgPadding.left
            }
            val numericY = if (orientation == Configuration.ORIENTATION_LANDSCAPE) {
                // Для альбомной ориентации
                (key.height - keyBgPadding.top - keyBgPadding.bottom) / 2F -
                        2 * paint.textSize + (paint.textSize - paint.descent()) / 2F + keyBgPadding.top
            } else {
                // Для портретной ориентации
                (key.height - keyBgPadding.top - keyBgPadding.bottom) / 2F -
                        2 * paint.textSize + (paint.textSize - paint.descent()) / 2F + keyBgPadding.top
